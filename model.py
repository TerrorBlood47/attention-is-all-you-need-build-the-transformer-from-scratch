...
def run_training_loop_for_steps(batches, parameter_list, model_params, optimizer_state, num_steps, config):
    ...
    for step in range(1, num_steps+1):
        ...
        batch_loss = run_training_step_with_backprop(src_batch, tgt_batch, parameter_list, model_params,\
        optimizer_state, step, config)
...
def select_top_k_candidates(candidate_scores, k):
    ...
    n_batch, V = candidate_scores.shape

    flattend_candidate_scores = candidate_scores.flatten()

    scores , indices = torch.topk(flattend_candidate_scores, k=k, largest=True, sorted=True)
    ...
    return {
        "beam_indices" : torch.tensor(beam_indices, dtype=torch.long),
...
def append_tokens_to_beam_sequences(beam_sequences, beam_indices, token_ids):
    ...
    next_beam_sequences = torch.cat([parent_rows, token_ids.unsqueeze(-1)], dim=1).to(dtype=beam_sequences.dtype)
...
def select_best_finished_beam(finished_sequences, finished_scores, alpha):
    ...
    for i, seq in enumerate(finished_sequences):
        ...
        seq_len_i = len(seq)
        si = finished_scores[i]

        norm_score = si / compute_length_penalty(seq_len_i, alpha)
